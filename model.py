...
from models.sysnthesis_transform import Synthesis_transform
from models.ballenet import Hyper_analysis
from models.ballenet import Hyper_synthesis
import time
...
class ImageCompressor(nn.Module):
    def __init__(self, out_channel_N=128):
        ...
        self.phi1 = nn.Conv2d(2 * out_channel_N, out_channel_N, 3, stride=1, padding=1)
        self.phi_relu = nn.LeakyReLU()
        self.phi2 = nn.Conv2d(out_channel_N, out_channel_N, 3, stride=1, padding=1)
        # self.out_channel_M = out_channel_M

    def forward(self, input_image):
        quant_noise_feature = torch.zeros(input_image.size(0), self.out_channel_N, input_image.size(2) // 16,
                                          input_image.size(3) // 16).to("cuda:0")
        quant_noise_z = torch.zeros(input_image.size(0), self.out_channel_N, input_image.size(2) // 64,
                                    input_image.size(3) // 64).to("cuda:0")
        ...
        # print(f'编码时间:{end_time-start_time:.3f} s')
        att_out = feature
        ...
        dec_time = de_end_time - de_start_time
        # print(f'解码时间:{de_end_time - de_start_time:.3f} s')
        ...
        return clipped_recon_image, mse_loss, bpp_feature, bpp_z, bpp #, att_out, enc_time, dec_time
...
def test_gaussian():
    ...
    gaussian = torch.distributions.laplace.Laplace(mu, sigma)
    probs = gaussian.cdf(data + 0.5) - gaussian.cdf(data - 0.5)
    print(probs)
...
if __name__ == "__main__":
    ...
    # test = Attention(num_filters=128)

    # 随机生成输入张量
    input_tensor = torch.randn(1, 3, 256, 256)
    # test_dataset = TestKodakDataset(data_dir='/media/zll/d1/Repo/wxj/image_compression/datasets/DOTA2/val/')
